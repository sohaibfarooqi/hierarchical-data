...
def build(ctx, type=None):
    ...
    if type == 'aj':
        last_id = db.session.query(AdjcencyListModel).order_by(
            AdjcencyListModel.id.desc()).first()
        start, end_range, parent_id, path = getMeta(last_id)
        insertAdjecencyList(last_id, start, end_range, parent_id)

    elif type == 'mp':
        last_id = db.session.query(MaterializedPathModel).order_by(
            MaterializedPathModel.id.desc()).first()
        start, end_range, parent_id, path = getMeta(last_id)
        insertMateriallizedPath(last_id, start, end_range, parent_id, path)

    elif type == 'ns':
        last_id = db.session.query(NestedSetModel).order_by(
            NestedSetModel.id.desc()).first()
        start, end_range, parent_id, path = getMeta(last_id)
...
def insertNestedSet(last_id, start_range, end_range, parent_id, parent_path):
    ...
    for child in tree:
        childs = set(
            [child_node.id for child_node in tree if child_node.parent_id == child.id])
        if childs.__len__() > 0:
...
def dfs(graph, start, visited=None):
    ...
    if start not in graph:
        ...
        if graph[start].issubset(visited):
            node = NestedSetModel.query.filter(
                NestedSetModel.id == start).first()
            LEFT = LEFT + 1
            node.rgt = LEFT
            db.session.add(node)
            db.session.commit()
